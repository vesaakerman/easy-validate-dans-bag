...
package object structural {
  ...
  def metadataFileMustContainDatasetAndFiles(b: BagDir) = Try {
    if (bagMustContainMetadataDirectory(b).isSuccess) {
      val pathOfMetadata = b.resolve("metadata").toRealPath()

      if (!Files.exists(pathOfMetadata.resolve("dataset.xml")))
        fail(" Mandatory file 'dataset.xml' is not found in metadata file ")

      if (Files.exists(pathOfMetadata.resolve("dataset.xml")))
        pathOfMetadata.resolve("dataset.xml").toRealPath().toFile.delete

      if (!Files.exists(pathOfMetadata.resolve("files.xml")))
        fail(" Mandatory file 'files.xml' is not found in metadata file ")

      if (Files.exists(pathOfMetadata.resolve("files.xml")))
        pathOfMetadata.resolve("files.xml").toRealPath().toFile.delete

      if (pathOfMetadata.toFile.listFiles().nonEmpty)
